...
def compute_avgcodelength(logs):
    ...
    for idx in range(len(logs)):
        avg_codelen+= es_list[idx]*(idx+1)
    avg_codelen = (avg_codelen + 10* ((2048*16) - sum(es_list)))/(2048*16)
    return avg_codelen
...
def train_model(model, args):
    ...
    model.train()
    fadingLoader = DataLoader(args.train, args.fading_process, args.sigma1, args.sigma2)
    ...
    if args.resume is not None:
        ...
        if args.train and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint and args.start_step!=0:
    ...
    for eachbatch in range(args.start_step,args.total_iter):
        ...
        fw_h, fb_h = generate_fading(args)
        ...
        fwd_noise_par = torch.normal(0, std=std1,
                                     ...
                                     requires_grad=False).to(args.device)/fw_h
        fb_noise_par = torch.normal(0, std=std2,
                                     size=(args.batchSize, args.numb_block, args.truncated),
                                     requires_grad=False).to(args.device)/fb_h
        if args.snr2 == 100:
        ...
        if np.mod(eachbatch, args.core * 20000) == args.core - 1 and eachbatch >= 40000:
            ...
            saveDir = 'weights/{}/model_weights_{}_{}_'.format(args.model_name,snr1,snr2) + str(eachbatch)
            torch.save(state_dict, saveDir)
        else:
            if not os.path.exists('weights/{}'.format(args.model_name)):
                os.mkdir('weights/{}'.format(args.model_name))
            torch.save(state_dict, 'weights/{}/latest'.format(args.model_name))
...
def evaluate_model(model, args):
    ...
    for eachbatch in range(args.numTestbatch):
        ...
        if args.multclass:
            ...
            ys = bVec.long().contiguous().view(-1)
        with torch.no_grad():
            test_log,preds,comm_rounds = model(args.belief_threshold, eachbatch, bVec.to(args.device), fwd_noise_par,fb_noise_par, ys,isTraining=0, fadingLoader=fadingLoader)
            codelen = compute_avgcodelength(test_log)
            comm_round = comm_rounds/args.batchSize

            avg_codelen = avg_codelen*eachbatch/(eachbatch+1) + codelen/(eachbatch+1)
            avg_comm_round = avg_comm_round*eachbatch/(eachbatch+1) + comm_round/(eachbatch+1)
            Rd = Rd*eachbatch/(eachbatch+1) + (avg_comm_round - avg_codelen)/(eachbatch+1)
            ...
            if eachbatch > 100:
                ...
                BER = bitErrors / (eachbatch - 100) / args.batchSize / args.numb_block
                pktErrors += decisions.view(args.batchSize, args.numb_block).sum(1).count_nonzero()
                PER = pktErrors / (eachbatch - 100) / args.batchSize
                log = {"batch":eachbatch- 100,
                       "avg_codelen":avg_codelen,
                       'avg_comm_round':avg_comm_round,
                       'Rd':Rd,
                       "snr1":args.snr1,
                       "snr2":args.snr2,
                       "BER":BER.item(),
    ...
    BER = bitErrors.cpu() / ((args.numTestbatch-100) * args.batchSize * args.K)
    PER = pktErrors.cpu() / ((args.numTestbatch-100) * args.batchSize)
    print(BER)
    print(PER)
